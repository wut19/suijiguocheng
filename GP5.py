...
data1 = np.loadtxt(open("data5(AMDI).csv","rb"),delimiter="\",\"",skiprows=1,usecols=[4])
...
# 第一次进行核组合后，EX+SE的似然值最大，进行对其预测
predict_mean1_RC , predict_var1_RC = model1_RC1.predict(x2)
...
plt.plot(x3,y[::-1],'r')
# EX+SE
model1_RC1.plot(plot_limits=(395,410))
y = (data1[0:15] - train_data1_mean)/train_data1_sigma
plt.plot(x3,y[::-1],'r')
...
# plt.plot(x3,train_data1[360:400])
# y = (data1[0:15] - train_data1_mean)/train_data1_sigma
# plt.plot(x3,y[::-1],'r')
...
print("MSE1_RC:", MSE1_RC, " MAE1_RC:", MAE1_RC, " MAPE1_RC:", MAPE1_RC, " NOO1_RC:", NOO1_RC, " likelihood", model1_RC1.log_likelihood())
